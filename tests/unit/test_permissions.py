...
class TestPermissions:
    ...
    async def test_admin_can_access_any_report(self, services):
        ...
        report = await s["report_svc"].create(_stable_uuid("author-1"), "Secret Report")
        assert await s["perm_svc"].check(_stable_uuid("admin-1"), report.id, "owner")
    ...
    async def test_owner_has_all_permissions(self, services):
        ...
        report = await s["report_svc"].create(_stable_uuid("owner-1"), "My Report")
        for level in ("viewer", "commenter", "editor", "owner"):
            assert await s["perm_svc"].check(_stable_uuid("owner-1"), report.id, level)
    ...
    async def test_editor_can_edit_not_change_visibility(self, services):
        ...
        assert await s["perm_svc"].check(_stable_uuid("editor-1"), report.id, "editor")
        assert not await s["perm_svc"].check(_stable_uuid("editor-1"), report.id, "owner")
    ...
    async def test_viewer_cannot_edit(self, services):
        ...
        assert await s["perm_svc"].check(_stable_uuid("viewer-1"), report.id, "viewer")
        assert not await s["perm_svc"].check(_stable_uuid("viewer-1"), report.id, "commenter")
        assert not await s["perm_svc"].check(_stable_uuid("viewer-1"), report.id, "editor")
    ...
    async def test_group_access_grants_to_members(self, services):
        ...
        await s["group_repo"].add_member(group.id, _stable_uuid("member-1"))

        report = await s["report_svc"].create(_stable_uuid("owner-1"), "Report")
        await s["permission_repo"].set_group_access(report.id, group.id, "editor")

        assert await s["perm_svc"].check(_stable_uuid("member-1"), report.id, "editor")
    ...
    async def test_direct_access_overrides_group(self, services):
        ...
        group = await s["group_repo"].create(group)
        await s["group_repo"].add_member(group.id, _stable_uuid("user-1"))

        report = await s["report_svc"].create(_stable_uuid("owner-1"), "Report")
        await s["permission_repo"].set_group_access(report.id, group.id, "viewer")
        await s["permission_repo"].set_user_access(report.id, _stable_uuid("user-1"), "editor")

        # Should have editor (direct) not just viewer (group)
        assert await s["perm_svc"].check(_stable_uuid("user-1"), report.id, "editor")
    ...
    async def test_suspended_user_denied(self, services):
        ...
        # Sanction the user
        await s["mod_svc"].sanction(_stable_uuid("mod-1"), _stable_uuid("editor-1"), "suspend", "testing")

        assert not await s["perm_svc"].check(_stable_uuid("editor-1"), report.id, "editor")
    ...
    async def test_public_report_readable(self, services):
        ...
        report = await s["report_svc"].create(_stable_uuid("owner-1"), "Public Report")
        report.visibility = "public_auth"
        await s["report_repo"].update(report)

        assert await s["perm_svc"].check(_stable_uuid("random-1"), report.id, "viewer")
    ...
    async def test_private_report_invisible(self, services):
        ...
        report = await s["report_svc"].create(_stable_uuid("owner-1"), "Private Report")
        assert not await s["perm_svc"].check(_stable_uuid("outsider-1"), report.id, "viewer")
