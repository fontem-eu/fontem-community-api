...
Pins the contract that crawlers rely on:
  ...
  * `/sitemap-reports.xml` lists every `public_*` report.
...
class TestSitemap:
    def test_sitemap_index_lists_shards(self, client, services):
        ...
        assert any(u.endswith("/sitemap-reports.xml") for u in locs)
    ...
    def test_sitemap_reports_includes_only_public_reports(self, client, services):
        _seed(services)
        h = make_headers("user-1")

        # 1 private, 1 public_open, 1 public_auth — only the last two
        # should show up.
        private_resp = client.post("/reports", json={"title": "private"}, headers=h)
        assert private_resp.status_code == 201
        priv_id = private_resp.json()["id"]

        public_resp = client.post("/reports", json={"title": "public"}, headers=h)
        pub_id = public_resp.json()["id"]
        client.put(f"/reports/{pub_id}", json={"visibility": "public_open"}, headers=h)

        mixed_resp = client.post("/reports", json={"title": "authed"}, headers=h)
        mid = mixed_resp.json()["id"]
        client.put(f"/reports/{mid}", json={"visibility": "public_auth"}, headers=h)

        resp = client.get("/sitemap-reports.xml")
        assert resp.status_code == 200
        root = ET.fromstring(resp.text)
        locs = [el.text for el in root.findall(".//sm:url/sm:loc", NS)]
        ...
        assert any(u.endswith(f"/reports/{pub_id}") for u in locs)
        assert not any(u.endswith(f"/reports/{priv_id}") for u in locs)
    ...
    def test_sitemaps_are_anonymously_accessible(self, client, services):
        ...
        for path in ("/sitemap.xml", "/sitemap-core.xml", "/sitemap-reports.xml"):
