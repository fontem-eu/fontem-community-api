...
class ChatRequest:
    ...
    #: The article the user actually has open. The conversation key used to
    #: stand in for this, which held only while the two could not differ —
    #: pick an older thread in the switcher and the document tools pointed
    #: at whatever report THAT thread was about, in one production case a
    #: story that no longer existed. None for clients predating the field.
...
def _document_under_edit(req: ChatRequest) -> str | None:
    ...
    The one the user has open, when the client says so. The conversation
    key is only a fallback for clients that do not send it: it names the
    report a THREAD is about, which stops being the open article the
    moment somebody picks a different thread from the switcher. In
    production that pointed the tools at a deleted story, so every
    read_document answered "not found" while the author sat in a
    perfectly readable article and the model burned a long turn guessing
    why.
    """
    if req.report_id:
        return req.report_id
    if req.conversation_key.startswith("report:"):
        return req.conversation_key.split(":", 1)[1]
    return None
...
class AssistantService:
    ...
    async def turn(self, req: ChatRequest) -> AsyncIterator[str]:
        ...
        document_id = _document_under_edit(req)
        if self._reports is not None and document_id and payload["has_editor"]:
            ...
            payload["doc_ops"] = DocOps(self._reports, req.user_id, document_id)
