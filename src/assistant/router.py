...
class StudioQueryBody(BaseModel):
    """The query open in the Data Studio editor, as the editor has it.

    ``text`` is the CURRENT draft, not the saved query: the assistant is
    asked to fix what the user is looking at, which stops matching the
    saved copy the moment they type. ``last_error`` and ``columns`` come
    from the editor's last run, so the assistant can be told what failed
    or what came back without running anything itself.
    """

    id: str
    name: str = Field("", max_length=300)
    lang: str = Field(..., max_length=20)
    text: str = Field("", max_length=8000)
    last_error: str | None = Field(None, max_length=2000)
    columns: list[Annotated[str, Field(max_length=200)]] | None = Field(
        None, max_length=50)


class StudioContextBody(BaseModel):
    """Where in the Data Studio the user is. Sent only from a Studio view."""

    project_id: str
    project_name: str = Field("", max_length=300)
    query: StudioQueryBody | None = None
...
class AssistChatBody(BaseModel):
    ...
    studio: StudioContextBody | None = Field(
        None,
        description=(
            "The Data Studio project the user is in and, when one is open, "
            "the query editor's current text. Present only while a Studio "
            "view is registered; with an open query the assistant may "
            "propose new text for it, reviewed in the editor as a diff. "
            "Ignored on a signed-out turn."
        ),
    )
    ...
    *,
    ...
    if user is None:
        ...
        req = ChatRequest(
            ...
            # Ignored on the anonymous branch above, like has_editor: a
            # proposal needs an account to review it under.
            studio=body.studio.model_dump() if body.studio else None,
        )
