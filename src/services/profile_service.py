...
class ProfileService:
    ...
    async def get_profile(  # pylint: disable=too-many-arguments,too-many-positional-arguments
        ...
        public_email = (custom if (use_custom and custom) else user.email) if show_email else ""
    ...
    async def update_own_profile(  # pylint: disable=too-many-arguments,too-many-positional-arguments,too-many-locals
        ...
        for raw in (links or [])[:MAX_LINKS]:
            ...
            if normalised is None:
                continue
            clean_links.append(ProfileLink(name=lname, url=normalised[:MAX_LINK_URL]))
        ...
        # A custom address only takes effect when it's a valid email.
        ...
        # Name lives on the user record; update it in place when provided.
        current_name = None
        user = await self._users.get_by_id(user_id)
        if user is not None:
